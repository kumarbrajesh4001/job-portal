...
import { getSkillAndLocationsForCandidates } from '../../helpers/getSkillAndLocationsForDynamicUrl';

function FindCandidateSearches(props) {
  ...
  const requestToServer = (searchRequestJSON, page, sortBy) => {
    ...
    storeSearchRequest(searchRequestJSON, true);
    postRequest(
      `/search/candidate?page=${page}&sortby=${sortBy}
    `,
      searchRequestJSON
    ).then((response) => {
  ...
  useEffect(() => {
    if (!isStringEmpty(skillsAndLocations)) {
      const { skills, locations } =
        getSkillAndLocationsForCandidates(skillsAndLocations);
      setKeywords(skills);
      setSearchLocation(locations);
      handleSearchFormRequest(skills, locations);
    }
...
FindCandidateSearches.propTypes = {
  setLoggedInUserRole: PropTypes.func,
};
FindCandidateSearches.defaultProps = {
